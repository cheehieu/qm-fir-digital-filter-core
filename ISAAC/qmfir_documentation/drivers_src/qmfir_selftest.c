/*****************************************************************************
* Filename:          /proj/users/hmnguyen/EDKproject/qmfir_2new/drivers/qmfir_v1_00_a/src/qmfir_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the qmfir driver
...
XStatus QMFIR_SelfTest(void * baseaddr_p)
{
  ...
  QMFIR_mWriteSlaveReg0(baseaddr, 0, 1);
  Reg32Value = QMFIR_mReadSlaveReg0(baseaddr, 0);
  ...
  QMFIR_mWriteSlaveReg1(baseaddr, 0, 2);
  Reg32Value = QMFIR_mReadSlaveReg1(baseaddr, 0);
  ...
  QMFIR_mWriteSlaveReg2(baseaddr, 0, 3);
  Reg32Value = QMFIR_mReadSlaveReg2(baseaddr, 0);
  ...
  QMFIR_mWriteSlaveReg3(baseaddr, 0, 4);
  Reg32Value = QMFIR_mReadSlaveReg3(baseaddr, 0);
  ...
  {
    QMFIR_mWriteMemory(LocalBRAM+4*Index, 0xDEADBEEF);
    Mem32Value = QMFIR_mReadMemory(LocalBRAM+4*Index);
  ...
  Reg32Value = QMFIR_mReadReg(baseaddr, QMFIR_INTR_IPISR_OFFSET);
  xil_printf("   - IP (user logic) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear IP (user logic) interrupt status register\n\r");
  QMFIR_mWriteReg(baseaddr, QMFIR_INTR_IPISR_OFFSET, Reg32Value);
  Reg32Value = QMFIR_mReadReg(baseaddr, QMFIR_INTR_DISR_OFFSET);
  xil_printf("   - Device (peripheral) interrupt status : 0x%08x\n\r", Reg32Value);
  xil_printf("   - clear Device (peripheral) interrupt status register\n\r");
  QMFIR_mWriteReg(baseaddr, QMFIR_INTR_DISR_OFFSET, Reg32Value);
  xil_printf("   - enable all possible interrupt(s)\n\r");
  QMFIR_EnableInterrupt(baseaddr_p);
