/* $Id: xlltemac.h,v 1.3.2.1 2009/02/03 01:09:34 wyang Exp $ */
/******************************************************************************
*
 ...
 *
 * For a full description of TEMAC features, please see the hardware spec.
 ...
 * UDP/TCP header+data. With this hardware implementation, the IP header
 * checksum cannot be offloaded. Many stacks that support offloading will
 ...
 * header that must be taken into consideration. Readers should consult a
 * TCP/IP design reference for more details.
 *
 ...
 * Refer to the XPS_LL_TEMAC v2.00a hardware specification for more details.
 *
 ...
 * (XTE_EXT_MULTICAST_OPTION): Allow and perform address filtering more than 4
 * multicast addresses. Hardware requires to enable promiscuous mode
 * (XTE_PROMISCUOUS_OPTION) and disable legacy multicast mode
 * (XTE_MULTICAST_OPTION) for this feature to work. However, these two modes
 * are still supported with v2.00a hardware.
 *
 * <h2>Extended VLAN</h2>
 *
 * <h3>TX/RX VLAN stripping</h3>
 * (XTE_EXT_[T|R]XVLAN_STRP_OPTION) handles transmit/receive one VLAN tag
 * stripping in ethernet frames. To enable this option, hardware requires to
 * build with this feature and enable (XTE_FCS_INSERT_OPTION),
 * (XTE_FCS_STRP_OPTION) and disable (XTE_VLAN_OPTION). Supprts three modes,
 * -XTE_VSTRP_NONE   : no stripping
 * -XTE_VSTRP_ALL    : strip one tag from all frames
 * -XTE_VSTRP_SELECT : strip one tag from selected frames
 *
 * <h3>TX/RX VLAN translation</h3>
 * (XTE_EXT_[T|R]XVLAN_TRAN_OPTION) handles transmit/receive one VLAN tag
 * translation in ethernet frames. To enable this option, hardware requires to
 * build with this feature and enable (XTE_FCS_INSERT_OPTION),
 * (XTE_FCS_STRP_OPTION), and disable (XTE_VLAN_OPTION).
 *
 * <h3>TX/RX VLAN tagging</h3>
 * (XTE_EXT_[T|R]XVLAN_TAG_OPTION) adds transmit/receive one VLAN tag in
 * ethernet frames. To enable this option, hardware requires to build with this
 * feature and enable (XTE_FCS_INSERT_OPTION), (XTE_FCS_STRP_OPTION),
 * (XTE_JUMBO_OOPTION) and disable (XTE_VLAN_OPTION). Support four modes,
 * -XTE_VTAG_NONE    : no tagging
 * -XTE_VTAG_ALL     : tag all frames
 * -XTE_VTAG_EXISTED : tag already tagged frames
 * -XTE_VTAG_SELECT  : tag selected already tagged frames
 *
 ...
 * explicitly set using XLlTemac_SetOperatingSpeed() and must match the speed
 ...
 * Device information
 */
#define XTE_DEVICE_NAME     "lltemac"
...
#define XTE_EXT_MULTICAST_OPTION         0x00000800
/**< XTE_EXT_MULTICAST_OPTION specifies the TEMAC channel to receive frames
 *   sent to Ethernet addresses that are programmed into the BRAM Multicast
 *   Address Table.
 *   This driver sets this option to be dependent on HW configuration
 *   by default. */

#define XTE_EXT_TXVLAN_TRAN_OPTION       0x00001000
/**< XTE_EXT_RXVLAN_TRAN_OPTION specifies the TEMAC channel to enable transmit 
 *   VLAN translation.
 *   This driver sets this option to be dependent on HW configuration
 *   by default. */

#define XTE_EXT_RXVLAN_TRAN_OPTION       0x00002000
/**< XTE_EXT_RXVLAN_TRAN_OPTION specifies the TEMAC channel to enable receive
 *   VLAN translation.
 *   This driver sets this option to be dependent on HW configuration
 *   by default. */

#define XTE_EXT_TXVLAN_TAG_OPTION        0x00004000
/**< XTE_EXT_TXVLAN_TAG_OPTION specifies the TEMAC channel to enable transmit
 *   VLAN tagging.
 *   This driver sets this option to be dependent during HW build time
 *   by default. */

#define XTE_EXT_RXVLAN_TAG_OPTION        0x00008000
/**< XTE_EXT_RXVLAN_TAG_OPTION specifies the TEMAC channel to enable receive
 *   VLAN tagging.
 *   This driver sets this option to be dependent during HW build time
 *   by default. */

#define XTE_EXT_TXVLAN_STRP_OPTION       0x00010000
/**< XTE_EXT_TXVLAN_STRP_OPTION specifies the TEMAC channel to enable transmit
 *   VLAN stripping.
 *   This driver sets this option to be dependent during HW build time
 *   by default. */

#define XTE_EXT_RXVLAN_STRP_OPTION       0x00020000
/**< XTE_EXT_RXVLAN_STRP_OPTION specifies the TEMAC channel to enable receive
 *   VLAN stripping.
 *   This driver sets this option to be dependent during HW build time
 *   by default. */
...
/* Constant values returned by XLlTemac_GetPhysicalInterface(). Note that these
...
#define XTE_TPID_MAX_ENTRIES     4	/* Number of storable TPIDs in Table */

/* Constant values pass into XLlTemac_SetV[tag|Strp]Mode() and returned by
 * XLlTemac_GetV[tag|Strp]Mode().
 */
#define XTE_VTAG_NONE            0	/* no tagging */
#define XTE_VTAG_ALL             1	/* tag all frames */
#define XTE_VTAG_EXISTED         2	/* tag already tagged frames */
#define XTE_VTAG_SELECT          3	/* tag selected already tagged frames */
#define XTE_DEFAULT_TXVTAG_MODE  XTE_VTAG_ALL
#define XTE_DEFAULT_RXVTAG_MODE  XTE_VTAG_ALL

#define XTE_VSTRP_NONE           0	/* no stripping */
#define XTE_VSTRP_ALL            1	/* strip one tag from all frames */
#define XTE_VSTRP_SELECT         3	/* strip one tag from selected frames */
#define XTE_DEFAULT_TXVSTRP_MODE XTE_VSTRP_ALL
#define XTE_DEFAULT_RXVSTRP_MODE XTE_VSTRP_ALL
...
/**
 *
 * XLlTemac_IsStarted reports if the device is in the started or stopped state.
 * To be in the started state, the calling code must have made a successful
 * call to <i>XLlTemac_Start</i>. To be in the stopped state,
 * <i>XLlTemac_Stop</i> or <i>XLlTemac_CfgInitialize</i> function must have
 * been called.
 *
...
* @param InstancePtr references the TEMAC channel on which to operate.
*
* @return XLlTemac_IsDma returns XTRUE if the device is connected DMA.
*         Otherwise, XLlTemac_IsDma returns XFALSE.
*
* @note
*
...
* @param InstancePtr references the TEMAC channel on which to operate.
*
* @return XLlTemac_IsFifo returns XTRUE if the device is connected to a fifo
*         core. Otherwise, XLlTemac_IsFifo returns XFALSE.
*
* @note
*
...
/**
 *
 * XLlTemac_IsExtFuncCap determines if the device is capable of new/extend
 * VLAN and multicast features.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return TRUE if the device is capable and configured with new/extended VLAN
 *         and multicast features Otherwise, returns FALSE.
 *
 * @note
 * C-style signature:
 *     u32 XLlTemac_IsExtFuncCap(XLlTemac *InstancePtr)
 *
 *****************************************************************************/
#define XLlTemac_IsExtFuncCap(InstancePtr)                                   \
		((XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,        \
				XTE_RAF_OFFSET) & XTE_RAF_NEWFNCENBL_MASK) ? \
				TRUE : FALSE)

/*****************************************************************************/
/**
 *
 * XLlTemac_IsExtMcastEnable determines if the new/extended multicast features
 * is enabled.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return TRUE if the device new/extended multicast features is enabled.
 * Otherwise, returns FALSE.
 *
 * @note : This function indicates when extended Multicast is enabled in HW,
 *         extended multicast mode in wrapper can be tested.
 *
 * C-style signature:
 *     u32 XLlTemac_IsExtMcastEnable(XLlTemac *InstancePtr)
 *
 *****************************************************************************/
#define XLlTemac_IsExtMcastEnable(InstancePtr)                               \
		((XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,        \
			XTE_RAF_OFFSET) & XTE_RAF_EMULTIFLTRENBL_MASK) ?     \
				TRUE : FALSE)

/*****************************************************************************/
/**
 *
 * XLlTemac_IsExtMcast determines if the device is built with new/extended
 * multicast features.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return TRUE if the device is built with new/extended multicast features.
 * Otherwise, returns FALSE.
 *
 ...
 *
 * C-style signature:
 *     u32 XLlTemac_IsExtMcast(XLlTemac *InstancePtr)
 *
 *****************************************************************************/
...
/**
 *
 * XLlTemac_IsTxVlanStrp determines if the device is configured with transmit
 * VLAN stripping on the transmit channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsTxVlanStrp returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsTxVlanStrp returns FALSE.
 *
 * @note
 *
...
/**
 *
 * XLlTemac_IsRxVlanStrp determines if the device is configured with receive
 * VLAN stripping on the receive channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsRxVlanStrp returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsRxVlanStrp returns FALSE.
 *
 * @note
 *
...
/**
 *
 * XLlTemac_IsTxVlanTran determines if the device is configured with transmit
 * VLAN translation on the transmit channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsTxVlanTran returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsTxVlanTran returns FALSE.
 *
 * @note
 *
...
/**
 *
 * XLlTemac_IsRxVlanTran determines if the device is configured with receive
 * VLAN translation on the receive channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsRxVlanTran returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsRxVlanTran returns FALSE.
 *
 * @note
 *
 * Signature: u32 XLlTemac_IsRxVlanTran(XLlTemac *InstancePtr)
...
/**
 *
 * XLlTemac_IsTxVlanTag determines if the device is configured with transmit
 * VLAN tagging on the transmit channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsTxVlanTag returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsTxVlanTag returns FALSE.
 *
 * @note
 *
...
/**
 *
 * XLlTemac_IsRxVlanTag determines if the device is configured with receive
 * VLAN tagging on the receive channel.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return XLlTemac_IsRxVlanTag returns TRUE if the device is configured with
 *         VLAN trabslation on the transmit channel. Otherwise,
 *         XLlTemac_IsRxVlanTag returns FALSE.
 *
 * @note
 *
