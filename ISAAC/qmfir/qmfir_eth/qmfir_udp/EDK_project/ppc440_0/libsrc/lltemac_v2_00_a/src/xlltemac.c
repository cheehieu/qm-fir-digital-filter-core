/* $Id: xlltemac.c,v 1.3.2.1 2008/10/28 22:59:52 wyang Exp $ */
/******************************************************************************
*
 ...
 * XTE_RESET_HARD.
 *
 * XLlTemac_Reset also resets the TEMAC channel's options to their default
 * values.
 *
 ...
 *
 *        If XTE_RESET_HARD is set to XTE_RESET_HARD, then XLlTemac_Reset
 *        asserts the reset signal to the hard core block which will reset both
 *        channels of the TEMAC. This, of course, will bork any activity that
 *        may be occuring on the other channel. So, be careful here.<br><br>
 *
...
void XLlTemac_Reset(XLlTemac *InstancePtr, int HardCoreAction)
{
	...
	/*
	 * Check the status of the XTE_INT_MGT_LOCK bit in the interrupt status
	...
	while (TimeoutCount &&
		    (! (XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
		           XTE_IS_OFFSET) & XTE_INT_MGT_LOCK_MASK))) {
	...
	if (HardCoreAction == XTE_RESET_HARD) {
		...
		/*
		 * Check the status of the XTE_INT_MGT_LOCK bit in the
		 * interrupt status registers. This must be done to allow the
		 * MGT clock to become stable for the Sgmii and 1000BaseX PHY
		 * interfaces. No other register reads will be valid until this
		 * bit is valid.
		 * The bit is always a 1 for all other PHY interfaces.
		 */
		TimeoutCount = 4;
 ...
 * XTE_RAF_NEWFNCENBL_MASK bit dictats which offset will be configured.
 *
 ******************************************************************************/
int XLlTemac_SetMacAddress(XLlTemac *InstancePtr, void *AddressPtr)
{
	...
	if (!(XLlTemac_IsExtFuncCap(InstancePtr))) {
		...
		/*
	 	 * Set the MAC bits [31:0] in UNICASTADDR0
 		 * Having Aptr be unsigned type prevents the following
		 * operations from sign extending.
		 */
		XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
				XTE_UAWL_OFFSET, MacAddr);

		/* Set MAC bits [47:32] in UNICASTADDR1 */
		MacAddr  = 0;
		MacAddr |= Aptr[4];
		MacAddr |= Aptr[5] << 8;
		XLlTemac_WriteReg(InstancePtr->Config.BaseAddress,
				XTE_UAWU_OFFSET, MacAddr);
...
void XLlTemac_GetMacAddress(XLlTemac *InstancePtr, void *AddressPtr)
{
	...
	if (!(XLlTemac_IsExtFuncCap(InstancePtr))) {
		...
		/* Read MAC bits [31:0] in UNICASTADDR0 */
		MacAddr = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_UAWL_OFFSET);
		Aptr[0] = (u8) MacAddr;
		Aptr[1] = (u8) (MacAddr >> 8);
		Aptr[2] = (u8) (MacAddr >> 16);
		...
		/* Read MAC bits [47:32] in UNICASTADDR1 */
		MacAddr = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_UAWU_OFFSET);
		Aptr[4] = (u8) MacAddr;
		Aptr[5] = (u8) (MacAddr >> 8);
...
/*****************************************************************************/
/**
 * XLlTemac_UpdateDepOption check and update dependent options for
 * new/extended features. This is a helper function that is meant to be called
 * by XLlTemac_SetOptions() and XLlTemac_ClearOptions().
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
 *
 * @return DepOptions dependent options that are require to set/clear per
 *         hardware requirement.
 *
 * @note
 *
 * This helper function collects the dependent OPTION(s) per hardware design.
 * When conflicts arises, extended features have precedency over legacy ones.
 * Two operations to be considered,
 * 1. Adding extended options. If XTE_VLAN_OPTION is enabled and enable one of
 *    extended VLAN options, XTE_VLAN_OPTION should be off and configure to
 *    hardware.
 *    However, temac instance Options variable still holds XTE_VLAN_OPTION so
 *    when all of the extended feature are removed, XTE_VLAN_OPTION can be
 *    effective and configured to hardware.
 * 2. Removing extended options. Remove extended option can not just remove
 *    the selected extended option and dependent options. All extended options
 *    need to be verified and remained when one or more extended options are
 *    enabled.
 *
 * Dependent options are :
 * XTE_VLAN_OPTION, XTE_JUMBO_OPTION, XTE_FCS_INSERT_OPTION,
 * XTE_FCS_STRIP_OPTION and XTE_PROMISC_OPTION.
 *
 ******************************************************************************/
static u32 XLlTemac_UpdateDepOptions(XLlTemac *InstancePtr)
{
	/*
	 * This is a helper function for XLlTemac_[Set|Clear]Options(),
	 * verification has been done before invoke this funtion.
	 */
	...
	if (DepOptions & XTE_EXT_MULTICAST_OPTION) {
		/*
		 * when extended multicast module is enabled in HW,
                 * XTE_PROMISC_OPTION is required to be enabled.
		 */
		if (XLlTemac_IsExtMcast(InstancePtr)) {
			DepOptions |= XTE_PROMISC_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_TXVLAN_TRAN_OPTION) {
		/*
		 * Check if hardware is built with extend TX VLAN translation.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsTxVlanTran(InstancePtr)) {
			DepOptions |= XTE_FCS_INSERT_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_RXVLAN_TRAN_OPTION) {
		/*
		 * Check if hardware is built with extend RX VLAN translation.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsRxVlanTran(InstancePtr)) {
			DepOptions |= XTE_FCS_STRIP_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_TXVLAN_TAG_OPTION) {
		/*
		 * Check if hardware is built with extend TX VLAN tagging.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsTxVlanTag(InstancePtr)) {
			DepOptions |= XTE_FCS_INSERT_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			DepOptions |= XTE_JUMBO_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_RXVLAN_TAG_OPTION) {
		/*
		 * Check if hardware is built with extend RX VLAN tagging.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsRxVlanTag(InstancePtr)) {
			DepOptions |= XTE_FCS_STRIP_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			DepOptions |= XTE_JUMBO_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_TXVLAN_STRP_OPTION) {
		/*
		 * Check if hardware is built with extend TX VLAN stripping.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsTxVlanStrp(InstancePtr)) {
			DepOptions |= XTE_FCS_INSERT_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			DepOptions |= XTE_JUMBO_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	...
	if (DepOptions & XTE_EXT_RXVLAN_STRP_OPTION) {
		/*
		 * Check if hardware is built with extend RX VLAN stripping.
		 * if not, output an information message.
		 */
		if (XLlTemac_IsRxVlanStrp(InstancePtr)) {
			DepOptions |= XTE_FCS_STRIP_OPTION;
			DepOptions &= ~XTE_VLAN_OPTION;
			DepOptions |= XTE_JUMBO_OPTION;
			xdbg_printf(XDBG_DEBUG_GENERAL,
	 ...
	 * Return DepOptions to XLlTemac_[Set|Clear]Options for final
	 * configuration.
	 */
	return(DepOptions);
}
...
/*****************************************************************************/
/**
 * XLlTemac_SetOptions enables the options, <i>Options</i> for the TEMAC
 * channel, specified by <i>InstancePtr</i>. The TEMAC channel should be
 * stopped with XLlTemac_Stop() before changing options.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
...
int XLlTemac_SetOptions(XLlTemac *InstancePtr, u32 Options)
{
	...
	/* 
	 * There are options required to be on/off per hardware requirment.
	 * Invoke _UpdateDepOptions to check hardware availability and update
	 * options accordingly.
	 */
	DepOptions = XLlTemac_UpdateDepOptions(InstancePtr);

	/* New/extended function bit should be on if any new/extended features
	 * are on and hardware is built with them.
	 */
	if (DepOptions & (XTE_EXT_MULTICAST_OPTION   |
			  XTE_EXT_TXVLAN_TRAN_OPTION |
			  XTE_EXT_RXVLAN_TRAN_OPTION |
			  XTE_EXT_TXVLAN_TAG_OPTION  |
			  XTE_EXT_RXVLAN_TAG_OPTION  |
			  XTE_EXT_TXVLAN_STRP_OPTION |
			  XTE_EXT_RXVLAN_STRP_OPTION)) {
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
	...
	if (DepOptions & XTE_FLOW_CONTROL_OPTION) {
	...
	if (DepOptions & XTE_PROMISC_OPTION) {
	...
	if (DepOptions & XTE_BROADCAST_OPTION) {
		Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_RAF_OFFSET);
	...
	if (DepOptions & (XTE_MULTICAST_OPTION | XTE_EXT_MULTICAST_OPTION)) {
		Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_RAF_OFFSET);
	...
	if (DepOptions & XTE_EXT_TXVLAN_TAG_OPTION) {
		Reg = XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET);
		Reg = (Reg & ~XTE_RAF_TXVTAGMODE_MASK) |
		      (XTE_DEFAULT_TXVTAG_MODE << XTE_RAF_TXVTAGMODE_SHIFT);
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET, Reg);
	}
	...
	if (DepOptions & XTE_EXT_RXVLAN_TAG_OPTION) {
		Reg = XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET);
		Reg = (Reg & ~XTE_RAF_RXVTAGMODE_MASK) |
		      (XTE_DEFAULT_RXVTAG_MODE << XTE_RAF_RXVTAGMODE_SHIFT);
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET, Reg);
	}

	/* Enable extended transmit VLAN strip option */
	if (Options & XTE_EXT_TXVLAN_STRP_OPTION) {
		Reg = XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET);
		Reg = (Reg & ~XTE_RAF_TXVSTRPMODE_MASK) |
		      (XTE_DEFAULT_TXVSTRP_MODE << XTE_RAF_TXVSTRPMODE_SHIFT);
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET, Reg);
	}

	/* Enable extended receive VLAN strip option */
	if (Options & XTE_EXT_RXVLAN_STRP_OPTION) {
		Reg = XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET);
		Reg = (Reg & ~XTE_RAF_RXVSTRPMODE_MASK) |
		      (XTE_DEFAULT_RXVSTRP_MODE << XTE_RAF_RXVSTRPMODE_SHIFT);
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET, Reg);
	}
...
/*****************************************************************************/
/**
 * XLlTemac_ClearOptions clears the options, <i>Options</i> for the TEMAC
 * channel, specified by <i>InstancePtr</i>. The TEMAC channel should be
 * stopped with XLlTemac_Stop() before changing options.
 *
 * @param InstancePtr references the TEMAC channel on which to operate.
...
int XLlTemac_ClearOptions(XLlTemac *InstancePtr, u32 Options)
{
	...
	if (DepOptions & XTE_FLOW_CONTROL_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling flow control\n");
		Reg = XLlTemac_ReadIndirectReg(InstancePtr->Config.BaseAddress,
				XTE_FCC_OFFSET);
		Reg &= ~XTE_FCC_FCRX_MASK;
		XLlTemac_WriteIndirectReg(InstancePtr->Config.BaseAddress,
	...
	if (DepOptions & XTE_BROADCAST_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling broadcast mode\n");
		Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_RAF_OFFSET);
		Reg |= XTE_RAF_BCSTREJ_MASK;
		XLlTemac_WriteReg(InstancePtr->Config.BaseAddress, XTE_RAF_OFFSET,
	...
	if ((DepOptions & XTE_MULTICAST_OPTION) &&
	    (DepOptions & XTE_EXT_MULTICAST_OPTION)) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling multicast mode\n");
		Reg = XLlTemac_ReadReg(InstancePtr->Config.BaseAddress,
				XTE_RAF_OFFSET);
		Reg |= XTE_RAF_MCSTREJ_MASK;
		XLlTemac_WriteReg(InstancePtr->Config.BaseAddress, XTE_RAF_OFFSET,
	...
	if (DepOptions & XTE_EXT_MULTICAST_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling extended multicast mode\n");
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET) & ~XTE_RAF_EMULTIFLTRENBL_MASK);
	...
	if (DepOptions & XTE_EXT_TXVLAN_TAG_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling extended TX VLAN tag mode\n");
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET) & ~XTE_RAF_TXVTAGMODE_MASK);
	}

	/* Disable extended receive VLAN tag option */
	if (DepOptions & XTE_EXT_RXVLAN_TAG_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling extended RX VLAN tag mode\n");
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET) & ~XTE_RAF_RXVTAGMODE_MASK);
	}

	/* Disable extended transmit VLAN strip option */
	if (DepOptions & XTE_EXT_TXVLAN_STRP_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling extended TX VLAN strip mode\n");
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET) & ~XTE_RAF_TXVSTRPMODE_MASK);
	}

	/* Disable extended receive VLAN strip option */
	if (DepOptions & XTE_EXT_RXVLAN_STRP_OPTION) {
		xdbg_printf(XDBG_DEBUG_GENERAL, "Xtemac_ClearOptions: disabling extended RX VLAN strip mode\n");
		XLlTemac_WriteReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET,
			XLlTemac_ReadReg((InstancePtr)->Config.BaseAddress,
			XTE_RAF_OFFSET) & ~XTE_RAF_RXVSTRPMODE_MASK);
	}
