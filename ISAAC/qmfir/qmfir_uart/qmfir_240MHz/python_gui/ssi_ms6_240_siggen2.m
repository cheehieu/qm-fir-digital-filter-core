...
xpulse= cshift(x0',Nrngshft,Nrp);


% Upconverted to offset video

mod_angle= w_ov1*(0:Nrp-1).';
xpulse1= (10^(A1/20))*(real(xpulse).*cos(mod_angle) + imag(xpulse).*sin(mod_angle));
mod_angle= w_ov2*(0:Nrp-1).';
xpulse2= (10^(A2/20))*(real(xpulse).*cos(mod_angle) + imag(xpulse).*sin(mod_angle));
...
xn0r= max(xpulse2)*An*2*(rand(1,Nrp)-0.5);	% front-end thermal noise
xn0i= max(xpulse2)*An*2*(rand(1,Nrp)-0.5);	% front-end thermal noise
xn0= xn0r + ci*xn0i;
pxn0= fft(xn0,nfft0);
