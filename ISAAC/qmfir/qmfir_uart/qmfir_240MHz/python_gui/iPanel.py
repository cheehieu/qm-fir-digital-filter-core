...
exit""" % (new_freq)
                f1.close()
#                os.system("/proj/sw/Matlab/bin/matlab -nodesktop -nosplash -r rotate=%d <ssi_ms6_240_freq.txt" % (k*1600))
                old_freq = ((eval(frame.freqBox.GetValue())))
                print "old frequency = %d" % (old_freq)
...
exit""" % (eval(frame.freqBox.GetValue()))
                ...
                if k != 0:
                    print "   ---Generating new data file ---"
                    os.system("/proj/sw/Matlab/bin/matlab -nodesktop -nosplash -r rotate=%d <ssi_ms6_240_freq_re.txt" % ((k)*1600))    
...
# panel 11
        ...
        self.instInfo = "SMAP Digital Filter Demo Control Panel \n" +\
            "ISAAC iBoard \n" +\
            "Xilinx XC5VFX130T FPGA \n" +\
            "Demo Date: April 20, 2011"
